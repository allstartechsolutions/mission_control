...
export async function PATCH(request: Request, context: { params: Promise<{ id: string }> }) {
  try {
    const { id } = await context.params;
    const existing = await prisma.task.findUnique({ where: { id }, select: { id: true, clientId: true, projectId: true, cronLastRunAt: true } });
    ...
    const task = await prisma.task.update({
      ...
      data: {
        ...
        milestoneId,
        requesterEmployeeId,
        cronLastRunAt: cronFields.cronEnabled ? existing.cronLastRunAt : null,
        ...cronFields,
      },
    });
...
export async function DELETE(_request: Request, context: { params: Promise<{ id: string }> }) {
  try {
    const { id } = await context.params;
    const existing = await prisma.task.findUnique({ where: { id }, select: { id: true, clientId: true, projectId: true } });
    if (!existing) return NextResponse.json({ error: "Task not found." }, { status: 404 });
